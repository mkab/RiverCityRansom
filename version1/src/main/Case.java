...
public class Case extends JButton {
	...
	public BufferedImage ChargeImage( String image ){
		...
		try {
			return ImageIO.read(Thread.currentThread().getContextClassLoader().getResourceAsStream(image));
		} catch (IOException e) {
			e.printStackTrace();
			return BufFond;
	...
	public void UpdateG(){
		if(Modif){
			...
			for(int i=0; i<UpdateGraphicsTerrain.size();i++){
				g.drawImage(UpdateGraphicsTerrain.get(i).g,UpdateGraphicsTerrain.get(i).x,UpdateGraphicsTerrain.get(i).y, null);
			}
			for(int i=0; i<UpdateGraphicsPerso.size();i++){
				g.drawImage(UpdateGraphicsPerso.get(i).g,UpdateGraphicsPerso.get(i).x,UpdateGraphicsPerso.get(i).y, null);
			}
